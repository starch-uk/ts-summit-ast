...
import { parseApexSource } from '../../src/parser/apex/index.js';
 ...
 */
export function assertNoNodeOfType(
  root: ASTNode,
...
export function assertFullyTranslated(root: ASTNode): void {
  ...
  const untranslatedCount = countNodesOfType(
    root,
    (node) => node.kind === 'Untranslated' || node.kind === 'UntranslatedStatement' || node.kind === 'UntranslatedExpression'
  );
